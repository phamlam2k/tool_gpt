  ...
  return (
    <div className="py-[20px]">
      <div
        ...
        } border-t border-gray-600 pt-10 gap-4 p-4 max-md:container xl:w-[1300px] mx-auto`}
      >
        {questionList.data.data.map((data) => {
          return <PromptItem data={data} key={data.interactionId} />;
        })}
        ...
        {selectedPrompt && (
          ...
          >
            <div className="flex justify-end gap-[10px] my-[10px]">
              ...
              <p>-</p>
              <p>
                {extendedDayJs(selectedPrompt.timestamp).format("MMMM D, YYYY")}
              </p>
            </div>
            {/* Modal content goes here */}
            <h1 className="text-[20px] font-bold mb-[10px]">
              - Câu trả lời của ChatGPT -
            </h1>
            <div
              dangerouslySetInnerHTML={{
                __html: selectedPrompt.responseText.replaceAll("Copy code", ""),
              }}
      ...
      <div className="custom-pagination flex justify-center mt-[20px]">
        <Pagination
          ...
          total={questionList.data.totalItems}
        />
      </div>
    </div>
  );
